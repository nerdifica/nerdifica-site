  ...
  <footer class="mt-16 border-t border-border bg-surface">
    <div class="mx-auto max-w-6xl px-6 py-10">
      <div class="flex flex-col gap-10 md:flex-row md:justify-between">
        <div>
          <span class="inline-block rounded-control dark:bg-white dark:p-3">
        ...
        <nav aria-label="Footer">
          ...
          <ul class="mt-3 space-y-2 text-sm text-ink-700">
            <li>
              <NuxtLink :to="`/${locale}`" class="hover:text-primary-600 dark:hover:text-primary-400">
                {{ t('nav.home') }}
              </NuxtLink>
            </li>
            <li v-for="niche in niches" :key="niche.id">
              <NuxtLink :to="`/${locale}/${niche.slug[locale]}`" class="hover:text-primary-600 dark:hover:text-primary-400">
                {{ niche.name[locale] }}
              </NuxtLink>
            </li>
          </ul>
